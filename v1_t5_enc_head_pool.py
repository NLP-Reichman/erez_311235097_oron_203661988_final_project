...
# Dividing processes between GPUs (silences the barrier warning)
...
class T5EncoderMeanClassifier(nn.Module):
    """
    Encoder-only T5 classification with masked-mean pooling over encoder outputs.
    """
    def __init__(self, model_name: str, num_labels: int = 2, torch_dtype: Optional[torch.dtype] = None):
        super().__init__()
        ...
        # keep head parameters in same dtype as encoder params (e.g., bf16)
        enc_dtype = next(self.encoder.parameters()).dtype
        self.classifier.to(dtype=enc_dtype)
    ...
    def forward(
        ...
        **kwargs
        ...
        hidden = enc_out.last_hidden_state                                # [B,L,H]
        B, L, _ = hidden.shape
        if attention_mask is None:
            mask = torch.ones((B, L), device=hidden.device, dtype=hidden.dtype)
            lengths = torch.full((B, 1), L, device=hidden.device, dtype=hidden.dtype)
        else:
            mask = attention_mask.to(dtype=hidden.dtype)
            lengths = attention_mask.sum(dim=1, dtype=torch.float32).clamp_min(1.0).unsqueeze(-1).to(hidden.dtype)
        masked  = hidden * mask.unsqueeze(-1)                             # [B,L,H]
        pooled  = masked.sum(dim=1) / lengths                             # [B,H]
        # ensure pooled dtype matches classifier weights (bf16/fp16 vs fp32)
...
class T5EncoderAttnPoolClassifier(nn.Module):
    """
    Encoder-only T5 classification with masked attention pooling over encoder outputs.
    """
    def __init__(self, model_name: str, num_labels: int = 2, torch_dtype: Optional[torch.dtype] = None):
        super().__init__()
        self.encoder = T5EncoderModel.from_pretrained(
        ...
        # simple additive attention: score_i = u^T tanh(W h_i)
        self.attn_W = nn.Linear(H, H, bias=True)
        self.attn_u = nn.Linear(H, 1, bias=False)
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(
        ...
        **kwargs
        ...
        enc_out = self.encoder(input_ids=input_ids, attention_mask=attention_mask, return_dict=True)
        hs = enc_out.last_hidden_state                                    # [B,L,H]
        # Make activations match attention weights dtype (usually fp32)
        if hs.dtype != self.attn_W.weight.dtype:
            hs = hs.to(self.attn_W.weight.dtype)
        if attention_mask is None:
            mask = torch.ones(hs.size()[:2], device=hs.device, dtype=torch.bool)
        else:
            mask = attention_mask.bool()
        scores = self.attn_u(torch.tanh(self.attn_W(hs))).squeeze(-1)     # [B,L]
        # mask out padded tokens. use dtype-consistent -inf
        scores = scores.masked_fill(~mask, torch.finfo(scores.dtype).min)
        # if an example is fully masked, replace with zeros to avoid NaNs
        ...
        pooled = (attn * hs).sum(dim=1)                                   # [B,H]
        # Match classifier weight dtype (fp32 head, stable)
...
def run_1_t5_enc_head_pool_experiments(
    df: pd.DataFrame,
    label_col: str,
    task_def: str,
    model_name: str,
    ...
    if pooling == "masked_mean":
        base_classifier = T5EncoderMeanClassifier(model_name, num_labels=2, torch_dtype=dtype)
    elif pooling == "attention":
        base_classifier = T5EncoderAttnPoolClassifier(model_name, num_labels=2, torch_dtype=dtype)
    else:
    ...
    for ratio, suffix in exp_iter:
        ...
        if use_lora:
            if not PEFT_AVAILABLE:
                raise ImportError("use_lora=True but 'peft' is not installed.")
            peft_config = LoraConfig(
                ...
                # Safer across T5 impls
                # target_modules=["q", "k", "v", "o"],
                target_modules = ["q","k","v","o","wi","wo","wi_0","wi_1"],
            )
            classifier.encoder = get_peft_model(classifier.encoder, peft_config)
            if hasattr(classifier.encoder, "gradient_checkpointing_enable"):
                classifier.encoder.gradient_checkpointing_enable()
            if hasattr(classifier.encoder, "enable_input_require_grads"):
                classifier.encoder.enable_input_require_grads()
            torch.cuda.empty_cache()
            # ─── DEBUG: LoRA trainable parameters ─────────────────────────
            total_params     = sum(p.numel() for p in classifier.encoder.parameters())
            trainable_params = sum(p.numel() for p in classifier.encoder.parameters() if p.requires_grad)
        ...
        # 2. Tokenize in one shot (dynamic 'longest' padding)
        enc_tr = tokenizer(
            prompts_tr,
            max_length=max_length,
            padding=False,
        ...
        # Data collator
        # pad to multiples of 8 to better use tensor cores (bf16/fp16)
        ...
        with torch.no_grad():
            test_loss, test_logits = classifier(**test_batch)
        ...
        if trainer.is_world_process_zero():
            ...
            # Confusion matrix (predictions/labels are gathered on rank 0)
    ...
    # Plot confusion matrices & metrics
    for ratio, y_true, y_pred in cms:
        # if ratio is a float, format to 6 decimal places; otherwise just str()
        if isinstance(ratio, float):
            title = f"ratio={ratio:.6f}"
        else:
            title = f"ratio={ratio}"
    ...
    report_figs = []
    for ratio, y_true, y_pred in cms:
        report_figs.append(plot_confusion_matrix(y_true, y_pred,
                                                 title=f"ratio={ratio}"))
