...
class T5FusionClassifier(nn.Module):
    """
    Encoder+Decoder fusion head (concat):
      - encoder masked-mean pooled  → [B,H]
      - decoder last-layer, last tok → [B,H]
      - concat → [B,2H] → LN → Dropout → MLP → logits [B,C]
    """
    def __init__(self, model_name: str, num_labels: int = 2):
        super().__init__()
        self.t5 = T5Model.from_pretrained(model_name, torch_dtype=torch.bfloat16)
        self.hidden_size = self.t5.config.d_model
        self.num_labels = num_labels

        # normalize after concat (2H)
        self.fuse_ln = nn.LayerNorm(2 * self.hidden_size)
        ...
        self.bos_token_id = (
            ...
            if self.t5.config.decoder_start_token_id is not None
            else self.t5.config.pad_token_id
        )

        # train-time memory saver
    ...
    def forward(
        ...
        **kwargs
        ...
        # Drop collator extras (e.g., num_items_in_batch) & only pass T5-supported kwargs
        _ALLOWED_T5_KWARGS = {
        ...
        t5_kwargs: Dict[str, Any] = {k: v for k, v in kwargs.items() if k in _ALLOWED_T5_KWARGS and v is not None}
        # Explicitly ignore labels/num_items_in_batch/etc. for the encoder-decoder forward
        t5_kwargs.pop("labels", None)
        t5_kwargs.pop("num_items_in_batch", None)
        ...
        # encoder masked mean
        enc_last = outputs.encoder_last_hidden_state  # [B,L,H]
        B, L, _ = enc_last.shape
        if attention_mask is None:
            mask = torch.ones((B, L), device=enc_last.device, dtype=enc_last.dtype)
            lengths = torch.full((B, 1), L, device=enc_last.device, dtype=enc_last.dtype)
        else:
            mask = attention_mask.to(dtype=enc_last.dtype)
            # length sum in fp32 for numerical stability, then cast back
            lengths = attention_mask.sum(dim=1, dtype=torch.float32).clamp_min(1.0).unsqueeze(-1).to(enc_last.dtype)
        enc_sum = (enc_last * mask.unsqueeze(-1)).sum(dim=1)  # [B,H]
        enc_pooled = enc_sum / lengths                        # [B,H]

        # decoder last token (ensure hidden states are present)
        dec_hs = outputs.decoder_hidden_states
        if dec_hs is None:
            raise RuntimeError("decoder_hidden_states is None; set output_hidden_states=True.")
        dec_last_token = dec_hs[-1][:, -1, :]  # [B,H]

        # concat + LN + head
        fused = torch.cat([enc_pooled, dec_last_token], dim=-1)        # [B,2H]
        fused = self.fuse_ln(fused)
        head_dtype = next(self.classifier.parameters()).dtype
        fused = fused.to(head_dtype)
        # Ensure dtype matches classifier weights (avoid BF16/FP32 mismatch)
        fused = fused.to(self.classifier[0].weight.dtype)
...
def run_3_t5_fused_head_custom_experiments(
    df: pd.DataFrame,
    label_col: str,
    task_def: str,
    model_name: str,
    ...
    base_classifier = T5FusionClassifier(model_name, num_labels=2)
    ...
    for ratio, suffix in exp_iter:
        ...
        if use_lora:
            peft_config = LoraConfig(
                task_type=TaskType.FEATURE_EXTRACTION,   # full T5 (enc+dec)
                inference_mode=False,
                r=lora_r,
                lora_alpha=lora_alpha,
                lora_dropout=lora_dropout,
                # target_modules=["q", "k", "v", "o"],         # common for T5 blocks
                target_modules = ["q","k","v","o","wi","wo","wi_0","wi_1"],
            )
            classifier.t5 = get_peft_model(classifier.t5, peft_config)
            # memory savers
            classifier.t5.config.use_cache = False
            if hasattr(classifier.t5, "gradient_checkpointing_enable"):
                classifier.t5.gradient_checkpointing_enable()
            if hasattr(classifier.t5, "enable_input_require_grads"):
                classifier.t5.enable_input_require_grads()
            torch.cuda.empty_cache()
            # DEBUG: LoRA trainable parameters
            total_params     = sum(p.numel() for p in classifier.t5.parameters())
            trainable_params = sum(p.numel() for p in classifier.t5.parameters() if p.requires_grad)
        ...
        # Preprocess
        # train_data = preprocess_batch(df_tr, tokenizer, label_col, task_def, max_length)
        # val_data   = preprocess_batch(df_val, tokenizer, label_col, task_def, max_length)
        ...
        lengths = [f["input_ids"].size(0) for f in train_data]
